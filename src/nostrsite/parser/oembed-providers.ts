...
const providerUrls = OEMBED_PROVIDERS.map((p: any) => {
  return p.endpoints
    .map((e: any) => {
      return e.schemes?.map((s: any) => {
        return {
          r: new RegExp(s.replace("*", "[^./?]*")),
          u: e.url,
        };
      }) || [];
    })
...
export function getOembedUrl(url: string) {
  const purl = providerUrls.find((pu: any) => url.match(pu.r));
  console.log("oembed service url", purl, "for", url);
  if (purl)
    return purl.u + "?format=json&url=" + encodeURIComponent(url);
  else
    return "";
}
